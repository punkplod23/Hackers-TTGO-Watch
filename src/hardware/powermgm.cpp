...
void powermgm_loop( void ) {
    ...
    if ( powermgm_get_event( POWERMGM_SILENCE_WAKEUP_REQUEST | POWERMGM_WAKEUP_REQUEST ) ) {
        ...
        if ( lighsleep ) {
            log_i("go to 'lightsleep' standby");
            /*
             * set cpu speed
             * 
             ...
             */
            #if CONFIG_PM_ENABLE
                // Adjust power settings depending on if wifi or wifi+webserver is active
                if (wifictl_get_enable_on_standby() == true)
                {
                    log_i("wifictl is enabled on standby");
                    if(wifictl_get_webserver() == true)
                    {
                        // We need some more speed for the webserver
                        log_i("custom arduino-esp32 framework detected, enable PM/DFS support, 160/80MHz with light sleep");
                        pm_config.max_freq_mhz = 160;
                        pm_config.min_freq_mhz = 80;
                        pm_config.light_sleep_enable = true;
                    ...
                    {
                        // Seems like wifi stops working below 80MHz(?) 
                        log_i("custom arduino-esp32 framework detected - using 80MHz for background wifi with light sleep");
