...
void wifictl_setup( void ) {
        ...
        for( int i = 0 ; i < len ; i++ ) {
            for ( int entry = 0 ; entry < NETWORKLIST_ENTRYS ; entry++ ) {
                if ( !strcmp( wifictl_config.networklist[ entry ].ssid,  WiFi.SSID(i).c_str() ) && 
                      strcmp( wifictl_config.networklist[ entry ].ssid,  
                        wifictl_config.networklist_tried[ entry ].ssid ) ) 
                {
    ...
    WiFi.onEvent([](WiFiEvent_t event, WiFiEventInfo_t info) {
        ...
        wifictl_clear_event( WIFICTL_OFF_REQUEST | WIFICTL_ON_REQUEST | WIFICTL_SCAN );
    ...
    WiFi.onEvent([](WiFiEvent_t event, WiFiEventInfo_t info) {
        ...
        #endif
        wifictl_clear_event( WIFICTL_ACTIVE | WIFICTL_CONNECT | WIFICTL_OFF_REQUEST | WIFICTL_ON_REQUEST | WIFICTL_SCAN );
...
void wifictl_standby( void ) {
    ...
    */
    ...
    wifictl_off();
    while( wifictl_get_event( WIFICTL_ACTIVE | WIFICTL_CONNECT | WIFICTL_OFF_REQUEST | WIFICTL_ON_REQUEST | WIFICTL_SCAN ) ) { 
#ifndef NATIVE_64BIT
        yield();
#endif
    }
